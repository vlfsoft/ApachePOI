...
import vlfsoft.common.util.DatetimeUtils.asLocalDate
...
@SyntacticSugar
operator fun XSSFWorkbook.get(index: Int) = getSheetAt(index)
        ?: throw XSSFWorkbookGetSheetException("There no sheet for index = $index")

operator fun XSSFWorkbook.get(name: String) = getSheet(name)
        ?: throw XSSFWorkbookGetSheetException("There no sheet for name = '$name'")

class XSSFSheetGetRowException(message: String) : Exception(message)

fun XSSFSheet.getSafeRow(row: Int) = IndexedValue(row, getRow(row)
        ?: throw XSSFSheetGetRowException("Row[$row] is null"))
...
fun XSSFSheet.asSequence(startRowNum: Int = 0, endRowNum: Int = getRowEnd(), endOfStreamPredicate: (Row) -> Boolean = { true }): Sequence<IndexedValue<XSSFRow>> {
    var rowNum = startRowNum
    return generateSequence {
        if (rowNum <= endRowNum) {
            val r = getSafeRow(rowNum++); if (endOfStreamPredicate(r.value)) r else null
        } else null
    }
}
 ...
 *
 */
fun Row.getSafeCell(col: Int, itemName: String = "") = getCell(col)
        ?: throw XSSFSheetGetCellValueException("${itemName}value is null at column: $col")

fun IndexedValue<Row>.getSafeCell(col: Int, itemName: String = "") = value.getCell(col)
        ?: throw XSSFSheetGetCellValueException("${itemName}value is null at row: $index column: $col")

operator fun Row.get(col: Int, itemName: String = "") = getSafeCell(col, itemName)
operator fun IndexedValue<Row>.get(col: Int, itemName: String = "") = getSafeCell(col, itemName)
...
fun IndexedValue<Row>.stringCellValueWithEmptyAsNull(col: Int) = value.getCell(col)?.stringCellValue?.run { if (isNotEmpty()) this else null }
fun IndexedValue<Row>.stringCellValueWithNullAsEmpty(col: Int) = value.getCell(col)?.stringCellValue ?: ""

val Cell.stringOrLongCellValue: String
    get() =
        when (cellTypeEnum) {
            CellType.STRING -> stringCellValue
        // https://stackoverflow.com/questions/3148535/how-to-read-excel-cell-having-date-with-apache-poi
            CellType.NUMERIC -> if (HSSFDateUtil.isCellDateFormatted(this)) asLocalDate(dateCellValue).toString() else numericCellValue.toLong().toString()
            else -> ""
        }

fun IndexedValue<Row>.stringCellValueExWithEmptyAsNull(col: Int) = value.getCell(col)?.stringOrLongCellValue?.run { if (isNotEmpty()) this else null }
